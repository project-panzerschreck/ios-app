...
Coordinator IP/port load from disk **once at app launch** and save when you connect (manual edits are not overwritten by a stale connection URL). In-app logs retain the **latest 100 lines** only. See [BUILDING.md — Verbose RPC / GGML logging](BUILDING.md#verbose-rpc--ggml-logging) for log detail.
